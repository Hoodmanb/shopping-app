...
export default function Reciept({ transaction }) {
  ...
  let deliveredText = 'Your shipment was just dropped off. Go on. Open it and enjoy.';
  let shippedText = 'Your shipment is being shipped. You would be contacted soon, Thank you';
  let orderedText = 'Your order is confirmed, you would be contacted soon';
  ...
  const alignItems = {
    ...
    const DataBox = ({ data, sx }) => (
      <Box sx={sx}>
        {data.map((item, index) => (
          <div key={index} style={{marginBottom:'15px', width:'50%',}}>
            <h6 style={{ margin: 0, wordBreak: 'break-word',
              whiteSpace: 'normal', }}>{item.title}</h6>
            <p style={{ fontSize: '0.75rem', margin: 0, wordBreak: 'break-word', whiteSpace:'normal' }}>{item.content}</p>
          </div>
        ))}
      </Box>
    );
  ...
  return (
    ...
    <Box sx={{m:'10px'}}>
      ...
      <Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 0, mb:'20px' }}>
          {['Ordered', 'Shipped', 'Delivered'].map((label) => (
            <Box key={label} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', p: 0 }}>
        ...
        <Box sx={{...alignItems, borderRadius:'10px', backgroundColor:'grey', mb:'10px', pr:'5px', pl:'5px'}} key={product.id}>
        <Box sx={alignItems} key={product.id}>
          <img src={product.image}
          style={{width:'60px', height:'60px', borderRadius:'5px', marginRight:'20px'}}/>
          <Box>
            <h5>{product.name}</h5>
            <h6>Id: {product.id}</h6>
          </Box>
        ...
        <Divider sx={{mt:'20px'}}/>
        <Button sx={{ mt:'5px', mb:'5px',fontSize:'10px', color:'black'}}><LocalShippingIcon sx={{color:'blue'}}/> Track Shipment</Button>
        <Divider/>
      </Box>
      
    <Box className='footer'>
      {sections.map((section, index) => (
        <DataBox key={index} data={section.data} sx={{ ...alignItems, width: section.width, mt:'15px', mb:'15px' }} />
      ))}
