...
import ProductSkeleton from "@/app/components/Skeleton/Product"
...
function ImageGallery( {
  ...
  const [selectedImage,
    setSelectedImage] = useState(images[0] || "");

  if (images.length === 0) return <p>
    No images available
  </p>;
...
useEffect(() => {
  if (!isLoading && !error) {
    if (data?.message === "successful" && data?.product?._id) {
  ...
  const addToCart = async () => {
    ...
    try {
      const response = await axiosClient(`/api/cart/${id}`, "POST", {
